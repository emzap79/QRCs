...
\cmdM{subset(x, \ldots)}	{returns a selection of {\tt x} with respect to criteria ({\tt \ldots}, typically comparisons: {\tt x\$V1 < 10}); if {\tt x} is a data frame, the option {\tt select} gives the variables to be kept or dropped using a minus sign}
...
\cmdM{attributes(obj)}	{get or set the list of attributes of {\tt obj}}
...
\cmdM{substr(x,start,stop)}	{substrings in a character vector}
\cmdExmpl{\\substr(x, start, stop) <- value}	{can also assign, as}
\cmdM{strsplit(x,split)}	{split {\tt x} according to the substring {\tt split}}
...
\cmdM{xtabs(a~b,data=x)}	{a contingency table from cross-classi\-fy\-ing factors}
...
\cmdM{aov(formula)}	{analysis of variance model}
...
\cmdExmpl{rnorm(n, mean=0, sd=1)}	{Gaussian (normal)}
\cmdExmpl{rexp(n, rate=1)}	{exponential}
\cmdExmpl{rgamma(n, shape, scale=1)}	{gamma}
\cmdExmpl{rpois(n, lambda)}	{Poisson}
\cmdExmpl{rweibull(n, shape, scale=1)}	{Weibull}
\cmdExmpl{rcauchy(n, location=0, scale=1)}	{Cauchy}
\cmdExmpl{rbeta(n, shape1, shape2)}	{beta}
\cmdExmpl{rt(n, df)}	{`Student' ($t$)}
\cmdExmpl{rf(n, df1, df2)}	{Fisher--Snedecor ($F$)  ($\chi^2$)}
\cmdExmpl{rchisq(n, df)}	{Pearson}
\cmdExmpl{rbinom(n, size, prob)}	{binomial}
\cmdExmpl{rgeom(n, prob)}	{geometric}
\cmdExmpl{rhyper(nn, m, n, k)}	{hypergeometric}
\cmdExmpl{rlogis(n, location=0, scale=1)}	{logistic}
\cmdExmpl{rlnorm(n, meanlog=0, sdlog=1)}	{lognormal}
\cmdExmpl{rnbinom(n, size, prob)}	{negative binomial}
\cmdExmpl{runif(n, min=0, max=1)}	{uniform}
\cmdExmpl{rwilcox(nn, m, n)}	{{\tt rsignrank(nn, n)} Wilcoxon's statistics}
...
\cmdM{qqmath(\~{}x)}	{  quantiles of {\tt x} with respect to the values
