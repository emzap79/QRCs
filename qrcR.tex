...
\section{Variable information}{{\smalltt is.na(x), is.null(x), is.array(x), is.data.frame(x), \\is.complex(x), is.character(x), \ldots}}
\cmdS{methods(is)}	{all available lists of typetests}
\cmdS{methods(as)}	{list of variable conversion}
...
\cmdS{sort(x)}	{sorts the elements of {\tt x} in increasing order; to sort in decreasing order: {\tt rev(sort(x))}}
