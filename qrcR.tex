...
\subsubsection{matrices and data frames}{}
...
\cmdExmpl{id.}	{x\$name\or x[["name"]]}
\vfil\break

\section{Generating and Extracting Data}{}
...
\cmdM{grep(pttrn,x)}	{searches for matches to {\tt pattern} within {\tt x}; see {\tt ?regex}}
\cmdL{gsub(pttrn,replmt,x)}	{replacement of matches determined by regular expression matching {\tt sub()} is the same but only replaces the first occurrence.}
...
probability function, the cumulative probability function, and the value of
...
\subsection{Functions\ (y$func$)}{}
\cmdOper{Gaussian}{$y$norm(n, mean=0, sd=1)}
\cmdExmpl{pnorm(x)}{$\Phi(x)$}
\cmdExmpl{qnorm(x)}{$\Phi^{-1}(x)$}
\cmdOper{exponential}{$y$exp(n, rate=1)}
\cmdOper{gamma}{$y$gamma(n, shape, scale=1)}
\cmdOper{Poisson}{$y$pois(n, lambda)}
\cmdOper{Weibull}{$y$weibull(n, shape, scale=1)}
\cmdOper{Cauchy}{$y$cauchy(n, location=0, scale=1)}
\cmdOper{beta}{$y$beta(n, shape1, shape2)}
\cmdOper{`Student'-$t$}{$y$t(n, df)}
\cmdOper{Fisher-Snedecor}{$y$f(n, df1, df2)}
\cmdExmpl{}	{($F$)($\chi^2$)}
\cmdOper{Pearson}{$y$chisq(n, df)}
\cmdOper{geometric}{$y$geom(n, prob)}
\cmdOper{hypergeometric}{$y$hyper(nn, m, n, k)}
\cmdOper{logistic}{$y$logis(n, location=0, scale=1) }
\cmdOper{lognormal}{$y$lnorm(n, meanlog=0, sdlog=1)}
\cmdOper{uniform}{$y$unif(n, min=0, max=1)}
\subsection{Wilcoxon's statistics}{}
\cmdOper{$y$wilcox(nn, m, n)}{Wilcoxon Statistic}
\cmdOper{$y$signrank(nn, n)}{Signed Rank Statistic}
\subsection{Binomial Distributed}{}
\cmdOperL{$y$binom(n, size, prob)}{positive binomial}
\cmdOperL{$y$nbinom(n, size, prob)}{negative binomial}

\section{Time Series Calculations \rarrow library(xts)}{}
\cmdL{ts(x, start, end, frequency)}	{Create a time-series vector}
\cmdExmpl{\\ ts(rnorm(24), c(2010, 1), frequency=12)}	{Time Series Object
of 24 random variables over a two years monthly scale, starting from Jan 2010}
\cmdL{xts(x, order.by=as.POSIXct(\ldots))}	{Convert data vector to a timeseries-object}
\cmdExmpl{ar(x, order.max = 1, method = "ols")}	{fit an AR(1) to $x$ via
    an OLS regression}
...
\cmdExmpl{\\ window(x, start=c(2000, 11), frequency=T)}	{Resampling a timeseries for
    the month of November (every 11th month of a year) over the past couple of decades}
\cmdExmpl{window(x, c(2000,1), c(2015,12))}	{Extract data between Jan 2000 and Dec 2015 from x}
...
\cmdM{frequency(x)}	{returns the number of samples per unit time and deltat the
...
\subsubsection{Plot Modifiers}{The following parameters are common to many
    plotting functions}
\cmdOptM{add=F}	{if \T superposes the plot on the previous one (if it exists)}
\cmdOptM{axes=T}	{if \F does not draw the axes and the box}
\cmdOptM{type=""}	{ specifies the type of plot}
\cmdOper{"p"}{points}
\cmdOper{"l"}{lines}
\cmdOper{"b"}{points connected by lines}
\cmdOper{"o"}{id. but the lines are over the points}
\cmdOper{"h"}{vertical lines}
\cmdOper{"s"}{steps, the data are represented by the top of the vertical lines}
\cmdOper{"S"}{id. but the data are represented by the bottom of the vertical lines}

\cmdOptL{xlim=, ylim=}	{specifies the lower and upper limits of the axes}
\cmdExmpl{xlim=c(1, 10) \or xlim=range(x)}	{upper and lower limits}
\cmdOptL{xlab=, ylab=}	{ annotates the axes, must be variables of mode character}
\cmdOpt{main=}	{ main title, must be a variable of mode character}
\cmdOpt{sub=}	{ sub-title (written in a smaller font)}
...
\cmdOper{\tt cex.axis} {numbers on the axes}
\cmdOper{\tt cex.lab} {the axis labels}
\cmdOper{\tt cex.main} {the title}
\cmdOper{\tt cex.sub} {the sub-title}
