...
\def\versionnumber{0.2}  % Version of this reference card
...
% Workspace
\cmdS{who}	{displays a list of variables in the workspace}
\cmdS{whos}	{displays a detailed list of variables in the workspace}
%% Format Options
\cmdOper{format sets}	{the default format how $MATLAB$ displays numbers.}
\cmdOper{format short}	{5 digit fixed point}
\cmdOper{format long}	{15 digit fixed point}
\cmdOper{format short}	{e 5 digit floating point}
\cmdOper{format long}	{e 15 digit floating point}
%% Clear Variables \& Workspace
\cmdOper{clear x}	{clears the variable $x$}
\cmdOper{clear}	{erases all variables in the workspace}
\cmdOper{clc}	{clears the command window}
\cmdOper{close n}	{closes figure window no. $n$}
\cmdOper{close all}	{closes all figure windows}
...
% Data Creation
\cmdS{x=[1,2,4,...]}	{define a row vector $x$}
\cmdS{x=[1 2 4 ...]}	{same.}
\cmdS{x=[1; 2; 5; ...]}	{define a column vector $x$}
\cmdS{a:c}	{the range $a..c$; equivalent to $[a,a+1,...,c-1,c]$}
\cmdS{a:b:c}	{the range $a..c$ with step size $b$; equivalent to $[a,a+b,a+2*b,..., c-b,c]$ }
\cmdS{linspace(a,b,n)}	{a row vector with $n$ values linearly spaced from $a$ to $b$ (inclusive)}
...
\cmdS{sort(x)}	{sorts the elements of $x$ in ascending order}
...
\cmdS{reshape(x,m,n)}	{returns the $m  \times  n matrix$ whose elements are taken columnwise from $x$.}
...
% Math
\cmdS{sin,cos,tan,asin,acos,atan,atan2,log,log10,exp,...}	{These are the standard mathematical functions; they always operate pointwise on their arguments.}
...
\cmdOper{pi}	{$\pi = 3.1415926535897$ \ldots}
...
\cmdOper{realmax}	{Largest positive floating point number in $MATLAB$.}
\cmdOper{realmin}	{Smallest positive floating point number in $MATLAB$.}
\cmdOper{intmax}	{Largest positive integer value in $MATLAB$.}
\cmdOper{intmin}	{Smallest integer value in $MATLAB$.}
\cmdOper{eps}	{Spacing of floating point numbers. Use it to prevent unwanted behavior due to rounding errors. (See help for details.)}
...
\cmdS{filter(b,a,x)}	{filters the data in vector $x$ with the filter described by vectors $a$ and $b$.}
...
\cmdS{downsample(x,n)}	{downsamples the signal $x$ by keeping every nth sample starting with the first.}
\cmdS{upsample(x,n)}	{upsamples the signal $x$ by inserting $n?1$ zeros between input samples.}
\cmdS{resample(x,p,q)}	{resamples the signal $x$ at $p/q$ times the original sample rate.}

% Communication Toolbox
\cmdS{randint(m,n)}	{generates an $m \times n$ matrix of random binary numbers.}
\cmdS{randint(m,n,p)}	{generates an $m \times n$ matrix of random integers between $0$ and $p-1$.}
\cmdS{pskmod,pskdemod}	{phase shift keying modulation/demodulation}
\cmdS{qammod,qamdemod}	{quadrature amplitude modulation/demodulation}
...
% Sparse Matrices
% % Using sparse matrices can result in a significant computational gain if
% % you work with large matrices that have relatively few non-zero entries.
...
\cmdS{plot(x,y,...)}	{allows you to specify formatting options (cf. help plot)}
\cmdS{hist(x)}	{histogram of the frequencies of $x$}
\cmdS{stem(...)}	{is the same as $plot(...)$, but the data sequence is plotted as discrete "stems" from the x-axis with circles for the data values.}
\cmdS{semilogy(...)}	{is the same as $plot(...)$, except a logarithmic (base 10) scale is used for the y-axis.}
\cmdS{scatterplot(x)}	{generates a scatter plot of $x$. $x$ can be a real or complex vector, or a two-column matrix with real signal in the first column and imaginary signal in the second column.}
...
\cmdS{figure('name', '...')}	{creates a new figure window with the specified window title}
\cmdS{subplot(m,n,k)}	{divides the current figure window into $m \times n$ subfigures and selects the $kth$ for the current plot.}
\cmdS{xlabel('...')}	{sets the text for the x-axis. $xlabel$, as well as
$ylabel$, title etc. accept basic LATEX-like strings such as $a$\expon $2$ for
$a^2$ or \bs $alpha$ for $\alpha$.}
\cmdS{ylabel('...')}	{sets the text for the y-axis.}
\cmdS{title('...')}	{sets a title for the current plot.}
\cmdL{print -depsc2 fig.eps}	{saves the current figure into the file $fig.eps$.}

% Input and Output
\cmdS{input('prompt', fmt)}	{shows prompt for user input}
...
\cmdS{fprintf(fmt, vars, ...)}	{Like the C function $printf$}
% % formatstrings
%     %d  integer
...
\cmdOper{isnumeric(), ischar()}	{tests whether content of $x$ is numeric or a character textstring (boolean logic).}
\cmdS{sprintf(fmt, vars, ...)}	{Like $printf$, but returns the string instead of printing it to the screen.}
\cmdS{error('...')}	{displays an error message and halts execution. The message
can also be a formatting string as for $fprintf$, followed by the corresponding
variables, e.g. error('Warning \bs \%d\bs n', val).}

\cmdS{warning('...')}	{Like error, but execution of the function/script is continued. }
...
% % this structure will have a field for each variabe in the file. For example,
% % if $foo.mat$ contains variables $x$ and $y$, and you load the file with a =
% % load('foo'), then $x$ and $y$ will be accessible as $a.x$ and $a.y$.
\cmdS{save foo a b}	{... saves the variables $a, b$, etc. in the file $foo.mat$.}
\cmdS{save('foo', 'a', 'b')}	{same.}
