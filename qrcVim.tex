...
\section{Movements}	{{\tt :viu\grey{sage} } show a summary of all commands }
...
\cmdOper{;\ }	{the cursor position will be set to line of last search, substitution}
\cmdExmpl{, (this line) and ; (that line)}	{both {\tt ,} and {\tt ;} seperate line numbers. they differ in interpretation though}
...
\cmdOper{\bs \& }	{a `branch' matches last concat, but only if all preceding concats also match at the same position}
...
\cmdExmpl{/\/\bs(\#.*\bs)\bs@<=pattern}	{everything which comes before the
comment sign `\#' gets ignored by pattern}
...
\subsection{Search, Substitute Flags}	{refer to {\tt :h :s\_flags}}
...
\cmdExmpl{:\&r\  (same as \\:s//$c$\or:\tild)}	{repeat last search, substitution with same pattern string}
...
\cmdS{:ar\grey{gs} $f_1 \ldots f_n$}	{load files to buffer in background}
\cmdS{:$r$w\grey{rite} $f$ }	{write range $r$ to file $f$ (this file if no $f$)}
\cmdS{:$r$w\grey{rite}\kern2pt$>>f$ }	{append range $r$ to file $f$}
\cmdS{:q\grey{uit} :q\grey{uit}! }	{quit and confirm, quit and discard changes}
...
\cmdL{:$r$co\grey{py}\ $a$ :$r$m\grey{ove}\ $a$ }	{copy, move range $r$ below line $a$}
