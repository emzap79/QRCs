...
\centerline{\smallrm QRC of most often used Bash commands -- Version \version}}
...
This TeXfile is based on Michael Goerz (github.com/zapster)
and was then modified to my own use.
\endgroup}

% \pdfoutput=1
\pdfpageheight=21cm
\pdfpagewidth=30.2cm  % 29.7cm
...
%}}}
\def\gray#1{\pdfliteral{0.35 0.35 0.35 rg}#1\pdfliteral{0 0 0 rg}}
\def\black#1{\pdfliteral{0 0 0 rg}#1\pdfliteral{0.35 0.35 0.35 rg}}
% add greyline%{{{

% http://tug.org/pipermail/xetex/2012-February/022768.html
...
%}}}
\subsection{History-Commands}{example command is: \gray{{\it cd foo/bar}}}%{{{
...
\cmdExample{!?sudo?}{sudo cd foo/bar}
\cmd{!!:$des$}{appends first/last/specific/all $arg$'s of last command (trailing :$des$ refers to below listed designators)}
